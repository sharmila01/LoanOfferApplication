...
INSERT INTO `product` VALUES (1,'Smart Watch',1,2,1200),(2,'Samsung Galaxy 5S',2,2,1500);
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_order_detail_request_proc`(
IN pProductId int,
IN pQty int,
OUT rProductId int,
OUT rCategoryName varchar(50),
OUT rBrandName  varchar(50),
OUT rQty int,
OUT rProductName varchar(50),
OUT rSubTotal double
)
BEGIN
Declare lSalePrice double default 0;
	select
    p.ProductId,p.ProductName,p.SalePrice,c.CategoryName,b.BrandName
    into
...
-- Dump completed on 2023-02-18 17:13:26
